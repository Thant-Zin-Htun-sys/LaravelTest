    ...
    <div class="card-body">
        ...
        <form action="{{ route('movies.update', $movie->id) }}" method="POST">
            ...
            <div class="form-group mb-3">
                ...
                <input type="date" name="released_date" id="released_date" class="form-control @error('released_date') is-invalid @enderror" value="{{ old('released_date', $movie->released_date) }}" required>
                @error('released_date')
                    <div class="form-text text-danger">{{ $message }}</div>
                @enderror
            </div>
